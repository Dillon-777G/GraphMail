...
class PaginatedEmailService:
    ...
    self, folder_id: str, page: int = 1, per_page: int = 25) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self.retry_utils.retry_operation(retry_context)
            ...
            metrics.emails_processed = len(messages)
    ...
    async def get_cached_emails_by_ids(
        ...
        if len(cached_emails) == len(source_ids):
